...
void BoardRenderer::draw_board_perimeter() {
    ...
    draw_empty_rect(GUI::BLACK, left, top, right, bottom, m_layout.perimeter_width,
                    m_layout.perimeter_height);
}
...
void BoardRenderer::draw_cell(int row, int col, sf::Color const& color) {
...
void BoardRenderer::draw_hwall(int row, int col, sf::Color const& color) {
...
void BoardRenderer::draw_vwall(int row, int col, sf::Color const& color) {
...
void BoardRenderer::draw_walls(Board const& board) {
...
void BoardRenderer::draw_player(Cell const& position, sf::Color const& color, int offset_x,
                                int offset_y) {
...
void BoardRenderer::draw_players(Board const& board) {
    Cell red_pos = board.position(Player::Red);
    Cell blue_pos = board.position(Player::Blue);

    if (red_pos == blue_pos) {
        // Draw players with offset when on same cell
        draw_player(blue_pos, GUI::PLAYER2_COLOR, m_layout.shared_cell_offset_width,
                    m_layout.shared_cell_offset_height);
...
void BoardRenderer::draw_goals(Board const& board) {
...
void BoardRenderer::draw_reachable_cells(Board const& board, Player current_player,
                                         int actions_left) {
    ...
    if (actions_left >= 2) {
        sf::Color color_2 =
            (current_player == Player::Red) ? GUI::PLAYER1_COLOR : GUI::PLAYER2_COLOR;
        color_2 = increment_color(color_2, GUI::REACHABLE_CELLS_2ACTIONS_COLOR_INCREMENT);

        for (Cell const& cell : cells_2_actions) {
    ...
    if (actions_left >= 1) {
        ...
        for (Cell const& cell : cells_1_action) {
            draw_cell(cell.row, cell.column, color_1);
        }
    }
}

std::vector<Cell> BoardRenderer::get_cells_at_distance1(Board const& board, Player player) {
...
std::vector<Cell> BoardRenderer::get_cells_at_distance2(Board const& board, Player player) {
    std::vector<Cell> result;
    auto cells_1 = get_cells_at_distance1(board, player);

    for (Cell const& intermediate : cells_1) {
...
void BoardRenderer::highlight_cell(int row, int col) {
    ...
    draw_empty_rect(GUI::HIGHLIGHT_COLOR, left, top, right, bottom, m_layout.cell_highlight_width,
                    m_layout.cell_highlight_height);
}
...
void BoardRenderer::highlight_vwall(int row, int col) {
    ...
    draw_empty_rect(GUI::HIGHLIGHT_COLOR, left, top, right, bottom, m_layout.wall_highlight_width,
                    m_layout.wall_highlight_height);
}
...
void BoardRenderer::draw_empty_rect(sf::Color const& color, float left, float top, float right,
...
void BoardRenderer::write_text(std::string const& text, float left, float top,
